...
function populateModalItems(searchQuery = '') {
    ...
    const createClickableItem = (item) => {
        return `<li onclick="selectWasteItem('${item.replace(/'/g, "\\'")}')">${item}</li>`;
    };
    
    greenList.innerHTML = filterItems(wasteDatabase.green.items)
        .map(item => createClickableItem(item)).join('') || '<li>No items found</li>';
...
// Handle waste item selection from modal
function selectWasteItem(item) {
    // Close the modal
    closeItemsModal();
